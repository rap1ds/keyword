...
*/
function isRun(args) {
    var runCommands = splitBy(args, isKeywordName);
...
var localVars = function() {
    ...
    function set(key, val) {
        logger("Setting local variable '" + key + "': " + val);
        vars[key] = val;
    }

    function get(key) {
        logger("Getting local variable '" + key + "': " + vars[key]);
        return vars[key];    
    }
...
function lib(args) {
    ...
    keywords[name] = function() {
        ...
        logger('keywordInfo', keywordInfo);
        logger('vars', vars);

        var callback = function(returnVal) {
            logger('Keyword "' + name + '", callback info: ', JSON.stringify(args));
            next(returnVal);
        };

        logger('Calling def of keyword ' + name + " with args", JSON.stringify(args));
...
key.run = function() {
    ...
    run(_.toArray(arguments));
}

// Expose for testing purposes
key.helpers = Object.freeze({
    isPlainObject: isPlainObject,
    splitBy: splitBy
});

key.validation = Object.freeze({
    isSuite: isSuite,
    isDef: isDef,
    isLib: isLib,
    isRun: isRun,
