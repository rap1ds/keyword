...
var keyLib = pat()
    .caseof(Object, function(keywords) {
        _.forEach(keywords, function(keywordDefinition, name) {
            keyLib(keywordDefinition, name);
        });
    })
    .caseof(_.isFunction, validators.isKeywordName, function(fn, name) {
        libKeyword(fn, name);
    })
    .caseof(validators.isRun, validators.isKeywordName, function(keywordDefinition, name) {
