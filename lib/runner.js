...
function runKeyword(keyword, keywordsToRun, keywordsDefined, localVars, injector, then) {
    ...
    injector(keyword.name, def, defArgs);
}

function runKeywords(keywordsToRun, keywordsDefined, localVars, injector, then) {
    ...
    if(!keyword) {
        ...
        runKeyword(keyword, keywordsToRun, keywordsDefined, localVars, injector, then);
    }
}
...
/**
    ...
    [last]["vars"]: vars
*/
function run(keywordsToRun, keywordsDefined, injector, passedArguments) {
    var vars = helpers.createLocalEnv(passedArguments);
    keywordsToRun = helpers.createKeywords(keywordsToRun);

    return {
        then: function(callback) {
            runKeywords(keywordsToRun, keywordsDefined, vars, injector, callback);
        }
    };
}
